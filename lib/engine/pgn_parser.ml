...
let parse_san_move pos san_str =
  ...
  (* Handle castling *)
  if san_clean = "O-O" || san_clean = "0-0" then
    List.find_opt (fun mv -> Move.kind mv = Move.ShortCastle) legal_moves
  else if san_clean = "O-O-O" || san_clean = "0-0-0" then
    List.find_opt (fun mv -> Move.kind mv = Move.LongCastle) legal_moves
  else
    ...
    if len < 2 then None
      ...
      (try
         ...
         let disambig =
           if disambig_start >= 0 && disambig_start < len - 2 then
...
let parse_game_lines lines =
  ...
  let get_header tag =
    List.assoc_opt tag !headers |> Option.map String.trim |> function
    | Some "" -> None
    | x -> x
  in
  ...
  try
    while true do
      ...
      if line_trimmed = "" then
        (* Skip empty lines *)
        ()
      else if String.starts_with ~prefix:"[" line_trimmed then (
        (* Header line *)
        (* If we were already in a game and see a new header, finish previous game *)
        if !in_game && !current_game_lines <> [] && not (List.exists (fun l -> String.starts_with ~prefix:"[" l) !current_game_lines) then
          finish_current_game ();
        ...
        if String.contains line_trimmed '*'
