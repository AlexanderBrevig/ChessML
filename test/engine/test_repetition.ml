...
let test_avoids_repetition_when_winning () =
  ...
  let m1 = Move.make 0 1 Move.Quiet in (* a1 to b1 *)
  let game = Game.make_move game m1 in
  let m2 = Move.make 60 59 Move.Quiet in (* e8 to d8 *)
  let game = Game.make_move game m2 in
  let m3 = Move.make 1 0 Move.Quiet in (* b1 back to a1 - creates potential repetition *)
...
let test_seeks_repetition_when_losing () =
  ...
  let m1 = Move.make 60 59 Move.Quiet in (* e8 to d8 *)
  let game = Game.make_move game m1 in
  let m2 = Move.make 0 1 Move.Quiet in (* a1 to b1 *)
  let game = Game.make_move game m2 in
  let m3 = Move.make 59 60 Move.Quiet in (* d8 back to e8 - creates potential repetition *)
  let game = Game.make_move game m3 in
  ...
  Alcotest.(check bool)
    ...
    true
    (result.score > 0); (* Positive for white *)
;;
...
let test_game_history_tracking () =
  ...
  | move :: _ ->
    ...
    Alcotest.(check bool) 
      "Position keys differ after move"
      true
...
let test_threefold_repetition_detection () =
  ...
  | m :: _ ->
    ...
    (* Still no threefold *)
    Alcotest.(check bool) "No threefold after one move" false (Game.is_threefold_repetition game2);
;;
...
let () =
  ...
  run "Repetition" [
    ...
    "avoidance_when_winning", [
      test_case "Engine avoids repetition when winning" `Slow test_avoids_repetition_when_winning;
    ];
    "seeking_when_losing", [
      test_case "Engine considers repetition when losing" `Slow test_seeks_repetition_when_losing;
    ];
    "material_affects_incentive", [
      test_case "Material difference affects repetition incentive" `Quick test_material_affects_repetition_incentive;
    ];
    "history_tracking", [
      test_case "Game history is properly maintained" `Quick test_game_history_tracking;
    ];
    "threefold_detection", [
      test_case "Threefold repetition is correctly detected" `Quick test_threefold_repetition_detection;
    ];
  ]
